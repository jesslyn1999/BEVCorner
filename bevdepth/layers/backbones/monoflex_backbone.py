...
from bevdepth.layers.backbones.DCNv2.dcn_v2 import DCN

BN_MOMENTUM = 0.1

def build_backbone_monoflex(cfg):

    model = DLASeg(base_name=cfg['conv_body'],
...
class DLASeg(nn.Module):
    ...
    def forward(self, x):
        batch_size, num_sweeps, num_cams, num_channels, imH, imW = x.shape

        x = x.flatten().view(batch_size * num_sweeps * num_cams,
        ...
        out = y[-1]
        out = out.reshape(batch_size, num_sweeps, num_cams,
