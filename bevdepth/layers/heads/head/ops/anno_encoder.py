...
class Anno_Encoder():
	def __init__(
		...
		# center related
		self.num_cls = len(detect_classes)
		self.min_radius = min_radius
		self.max_radius = max_radius
		self.center_ratio = center_radius_ratio
		...
		self.depth_ref = torch.as_tensor(depth_reference).to(device=device)

		# dimension related
		self.dim_mean = torch.as_tensor(dimension_mean).to(device=device)
		self.dim_std = torch.as_tensor(dimension_std).to(device=device)
		self.dim_modes = dimension_reg
		...
		# offset related
		self.offset_mean = regression_offset_stat[0]
		self.offset_std = regression_offset_stat[1]
	...
	def update_device(self, device):
		self.depth_ref = self.depth_ref.to(device)
		self.dim_mean = self.dim_mean.to(device)
		self.dim_std = self.dim_std.to(device)
		self.alpha_centers = self.alpha_centers.to(device)


	def decode_box2d_fcos(self, centers, pred_offset, pad_size=None, out_size=None, batch_idxs=None):
		...
		if pad_size is not None:
			...
			box2d = box2d * self.down_ratio - pad_size[batch_idxs].repeat(1, 2)
	...
	def decode_box2d_fcos_ratio_same(self, centers, pred_offset, pad_size=None, out_size=None, batch_idxs=None):
		box2d_center = centers.view(-1, 2)
		box2d = box2d_center.new(box2d_center.shape[0], 4).zero_()
		# left, top, right, bottom
		box2d[:, :2] = box2d_center - pred_offset[:, :2]
		...
		if pad_size is not None:
			N = box2d.shape[0]
			# upscale and subtract the padding
			box2d = box2d - (pad_size[batch_idxs].repeat(1, 2) / self.down_ratio).long()
			# clamp to the image bound
	...
	def decode_location_flatten(self, points, offsets, depths, calibs, pad_size, batch_idxs):
		...
		points = (points + offsets) * self.down_ratio - pad_size[batch_idxs]  # why plus offset
	...
	def decode_dimension(self, cls_id, dims_offset):
		...
		'''
		self.update_device(cls_id.device)
