...
def main():
    ...
    trainval_nusc = NuScenes(version=version,
                             dataroot='{}/'.format(dataroot),
                             verbose=True)
    train_scenes = splits.train
    val_scenes = splits.val
    train_infos = generate_info(trainval_nusc, train_scenes)
    val_infos = generate_info(trainval_nusc, val_scenes)
    mmcv.dump(train_infos, '{}/nuscenes_infos_train.bevcorner2.pkl'.format(dataroot))
    mmcv.dump(val_infos, '{}/nuscenes_infos_val.bevcorner2.pkl'.format(dataroot))
    ...
    if data_type != 'mini':
        ...
        mmcv.dump(test_infos, '{}/nuscenes_infos_test.bevcorner2.pkl'.format(dataroot))
